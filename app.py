...
@app.route('/admin/backfill-visits', methods=['GET', 'POST'])
@login_required
def admin_backfill_visits():
    """ONE-TIME ROUTE: backfill visits from CSV data embedded here. Remove after use."""
    ...
    CSV_PATH = Path(__file__).parent / 'pipline_lazada_gabay(Sheet1) (1).csv'
    CURRENT_YEAR = 2026
    ...
    def _parse_date(s):
        ...
        for fmt in ('%Y/%m/%d','%d/%m/%Y','%m/%d/%Y','%Y-%m-%d'):
            try: return _dt2.strptime(s, fmt).date()
            except: pass
        for fmt in ('%d-%b','%d-%B'):
            try:
                d = _dt2.strptime(s, fmt)
                return _date2(CURRENT_YEAR, d.month, d.day)
            except: pass
        return None

    def _outcome(detail):
        d = (detail or '').lower()
        if 'registered' in d or 'live' in d: return 'registered'
        if 'negot' in d or 'interested' in d: return 'interested'
        if 'callback' in d or 'call back' in d: return 'callback'
        if 'no answer' in d or 'busy' in d: return 'no_answer'
        if 'revisit' in d or 'follow' in d: return 'follow_up'
        if 'not interested' in d or 'close' in d: return 'not_interested'
        return 'follow_up'

    logs = []
    preview = request.args.get('preview', '1') == '1'
    do_commit = request.method == 'POST'

    try:
        with open(CSV_PATH, encoding='utf-8-sig') as f:
            content = f.read()
    except FileNotFoundError:
        return '<h2>CSV file not found on server</h2>', 404
    ...
    for row in all_rows[2:]:
        ...
        exists = Visit.query.filter_by(lead_id=lead.id, gabay_id=agent.id).filter(
            func.date(Visit.visited_at) == visit_date).first()
        if exists:
            skipped['duplicate'] += 1; continue

        visited_at = _dt2.combine(visit_date, _dt2.min.time().replace(hour=9))
        to_insert.append(Visit(
            lead_id=lead.id, gabay_id=agent.id,
            visited_at=visited_at, outcome=_outcome(detail), notes=detail or None,
        ))
    ...
    if do_commit:
        for v in to_insert:
            db.session.add(v)
        db.session.commit()
        flash(f'✅ {len(to_insert)} visits committed to database!', 'success')
        return redirect(url_for('admin_backfill_visits') + '?preview=0')

    html = f"""
    <html><head><title>Backfill Visits</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    </head><body class="p-4">
    <h2>&#128202; Visit Backfill from CSV</h2>
    <table class="table table-bordered w-auto mb-4">
      <tr><td>Visits to INSERT</td><td><strong class="text-success">{len(to_insert)}</strong></td></tr>
      <tr><td>Skipped (no date)</td><td>{skipped['no_date']}</td></tr>
      <tr><td>Skipped (bad date)</td><td>{skipped['bad_date']}</td></tr>
      <tr><td>Skipped (no agent)</td><td>{skipped['no_agent']}</td></tr>
      <tr><td>Skipped (no lead)</td><td>{skipped['no_lead']}</td></tr>
    ...
    <table class="table table-sm table-bordered" style="font-size:12px">
      ...
      <tbody>
    {''.join(f"<tr><td>{v.lead_id}</td><td>{v.gabay_id}</td><td>{v.visited_at.date()}</td><td>{v.outcome}</td></tr>" for v in to_insert[:20])}
      </tbody>
    </table>
    <form method="POST">
      <button class="btn btn-danger btn-lg" onclick="return confirm('Commit {len(to_insert)} visits to the live database?')">
        &#9989; Commit {len(to_insert)} Visits Now
      </button>
